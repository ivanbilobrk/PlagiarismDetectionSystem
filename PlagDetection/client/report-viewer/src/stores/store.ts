...
const store = defineStore('store', {
  state: (): { state: State; uiState: UIState } => ({
    state: {
      ...
      resultHash: '',
      clusters: [] as Cluster[],
      topComparisons: [] as ComparisonListElement[],
      submissionFileIndex: {}
    },
    uiState: {
  ...
  getters: {
    ...
    getSubmissionIds(state): Array<string> {
      ...
      state.state.topComparisons.forEach(comparison => {
        submissionIds.add(comparison.firstSubmissionId);
        submissionIds.add(comparison.secondSubmissionId);
      });
