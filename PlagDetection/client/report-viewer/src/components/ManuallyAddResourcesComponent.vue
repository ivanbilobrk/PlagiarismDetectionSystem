  ...
  <div v-if="isOpen" class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50" @click.self="closeModal">
    <div class="bg-white p-8 rounded shadow-lg w-1/2">
      ...
      <div class="flex justify-center gap-4 mb-4">
        <button class="bg-blue-500 text-white py-1 px-3 rounded hover:bg-blue-600 flex items-center" @click="selectOption('local')">
        ...
        <button class="bg-blue-500 text-white py-1 px-3 rounded hover:bg-blue-600 flex items-center" @click="selectOption('git')">
        ...
        <button class="bg-blue-500 text-white py-1 px-3 rounded hover:bg-blue-600 flex items-center" @click="selectOption('ai')">
          <svg class="w-5 h-5 ml-1 mr-2" width="24" height="24" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg" fill="currentColor">
            <path d="M12 2a2 2 0 100 4 2 2 0 000-4zm-6 6h12a2 2 0 012 2v8a2 2 0 01-2 2h-1v2h-2v-2H9v2H7v-2H6a2 2 0 01-2-2v-8a2 2 0 012-2zm0 2v8h12v-8H6zm3 1h2v2H9v-2zm4 0h2v2h-2v-2z"/>
          </svg>
          AI
        </button>
      </div>
      <div v-if="selectedOption === 'local'">
        <AddZipResourceComponent :config-name="props.configName" :project-name="props.projectName" :subject-name="props.subjectName" @close="closeModal" />
      </div>
      <div v-if="selectedOption === 'git'">
        <GitAddResourceManuallyComponent :config-name="props.configName" :project-name="props.projectName" :subject-name="props.subjectName" @close="closeModal" />
      </div>
      <div v-if="selectedOption === 'ai'">
        <AIAddResourceManuallComponent :config-name="props.configName" :project-name="props.projectName" :subject-name="props.subjectName" @close="closeModal" />
      </div>
      <button class="mt-4 bg-gray-500 text-white py-1 px-3 rounded hover:bg-gray-600" @click="closeModal">
        Zatvori
      </button>
    </div>
  </div>
...
import AddZipResourceComponent from '@/components/AddZipResourceComponent.vue'
import GitAddResourceManuallyComponent from '@/components/GitAddResourceManuallyComponent.vue'
import AIAddResourceManuallComponent from '@/components/AIAddResourceManuallComponent.vue'
