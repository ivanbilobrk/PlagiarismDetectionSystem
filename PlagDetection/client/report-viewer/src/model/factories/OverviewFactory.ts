...
export class OverviewFactory extends BaseFactory {
   ...
   */
  public static extractOverview(json: Record<string, unknown>): OverviewExtractionResult {
  ...
  public static extractTopComparisons(
  ...
  public static extractClusters(clusters: Record<string, unknown>): Array<Cluster> {
    if (!clusters) {
      return []
    }

    const resolvedClusters = [] as Array<Cluster>
    for (const jsonCluster of clusters as Array<Record<string, unknown>>) {
